...
bool DemoApplication::BuildCommandBuffers() noexcept
{
    ...
    vkCmdResetQueryPool(primaryCmdBuffer, queryPools[0], 0, 2);

    vkCmdWriteTimestamp(primaryCmdBuffer, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, queryPools[0], 0);
    ...
    for (const auto& threadData : m_PerThreadData) {
        cmdBuffers.insert(cmdBuffers.end(),
                          threadData.secondaryCommandBuffers.begin(),
                          threadData.secondaryCommandBuffers.end());
    }

    vkCmdExecuteCommands(primaryCmdBuffer, cmdBuffers.size(), cmdBuffers.data());
